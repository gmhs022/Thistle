...
public class GeneralIO extends Device {
    ...
    private void utf8clear(int len, int data) {
        if (utf8buf[0] > 0) {
            for (int i = 0; i < utf8buf[1]; i++) console.write(utf8buf[i + 2] & 0xFF);
            for (int i = 0; i < utf8buf.length; i++) utf8buf[i] = 0;
        }
        utf8buf[0] = (byte) len;
        utf8buf[2] = (byte) data;
    }
    ...
    public int read(int address) {
        switch (address) {
            ...
            case GIO_INPUT_REG:
                if (inputbuf.size() > 0) return (inputbuf.remove() & 0xFF);
                else return 0;
        ...
        switch (address) {
            case GIO_INPUT_REG:
                if (data == 0) {
                    ...
                    while (data-- > 0) inputbuf.poll();
                }
                break;
            case GIO_OUTPUT_REG:
                if (data >= 128 && data < 192) { // Continuation
                    if (utf8buf[0] == 0) console.write(data);
                    else {
            ...
            case GIO_QUEUESTAT_REG:
                if (data == 0) {
                    ...
                    queuestat = this.getBus()
                        .getMachine()
                        .getContext()
                        .signal((String) tsfdata[0], Arrays.copyOfRange(tsfdata, 1, tsfdata.length)) ? 0 : 1;
                } else {
                    queuestat = 0xFF;
                }
                queuebuf.clear();
    ...
    public void load(NBTTagCompound nbt) {
        ...
        if (nbt.hasKey("genio")) {
            ...
            if (utf8buf.length == 6) this.utf8buf = utf8buf;
            signalbuf.clear();
            NBTTagCompound sigTag = genioTag.getCompoundTag("signals");
            int length = sigTag.getInteger("length");
            for (int i = 0; i < length; i++) signalbuf.add(sigTag.getByteArray("S" + i));
            inputbuf.clear();
    ...
    public void save(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < length; i++) sigTag.setByteArray("S" + i, signalbuf.get(i));
    ...
    public void onSignal(Signal signal) {
        ...
        if (name.equals("key_down")) {
            ...
            if (character == 0) {
                ...
                byte[] utf8 = String.valueOf(Character.toChars(character))
                    .getBytes(Charsets.UTF_8);
                for (byte data : utf8) inputbuf.add(data);
            }
            if ((nmimask & 1) != 0) cpu.assertNmi();
            else if ((irqmask & 1) != 0) cpu.assertIrq();
        } else if (name.equals("clipboard")) {
            byte[] paste = ((String) args[1]).getBytes(Charsets.UTF_8);
            if (paste.length > 0) {
                for (byte data : paste) inputbuf.add(data);
                if ((nmimask & 1) != 0) cpu.assertNmi();
                else if ((irqmask & 1) != 0) cpu.assertIrq();
            }
        }
        if (signalbuf.size() < 255) {
            ...
            if ((nmimask & 2) != 0) cpu.assertNmi();
            else if ((irqmask & 2) != 0) cpu.assertIrq();
        }
    }
    ...
    public void setBus(Bus bus) {
        super.setBus(bus);
        this.context = getBus().getMachine()
            .getContext();
