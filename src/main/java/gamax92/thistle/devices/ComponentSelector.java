...
public class ComponentSelector extends Device {
        ...
        if (delayConnectNBT != null) {
            for (int i = 0; i < components.length; i++) {
                if (delayConnectNBT[i] != null) {
                    mapComponent(
                        delayConnectNBT[i].getString("_address"),
                        i,
                        delayConnectNBT[i].getBoolean("_generic"));
    ...
    private Environment getEnvironment(int index) {
        ...
        if (component instanceof Environment) return (Environment) component;
        else if (component instanceof ThistleWrapper) return ((ThistleWrapper) component).host();
        return null;
    }
    ...
    private boolean mapComponent(String address, int select, boolean specific) {
        Node node = machine.node()
            .network()
            .node(address);
        if (node == null || !(node instanceof Component)) return false;
        ...
        if (wrapper != null) {
            try {
                components[select] = wrapper.getDeclaredConstructor(Environment.class)
                    .newInstance(host);
                return true;
            } catch (Exception e) {
                e.printStackTrace();
                ...
                switch (data) {
                    case 0: // map
                        ...
                        if (tsfdata == null || tsfdata instanceof Integer) {
                            ...
                            for (Map.Entry<String, String> entry : machine.components()
                                .entrySet()) {
                                if (entry.getValue()
                                    .equals(name)) {
                    ...
                    case 1: // unmap
                        ...
                        if (tsfdata == null) {
                            ...
                            for (int i = 0; i < components.length; i++) {
                                ...
                                if (environment != null && ((Component) environment.node()).name()
                                    .equals(name)) {
                                    info++;
                                    components[i] = null;
                                }
                    ...
                    case 3: // list
                        ...
                        if (inputbuf.size() > 0) {
                            ...
                            if (tsfdataz == null || tsfdataz.length != 1
                                || !(tsfdataz[0] instanceof String || tsfdataz[0] instanceof UUID
                                    || tsfdataz[0] instanceof Number)) {
                                status = 1;
                                break;
                            } else {
                                tsfdata = tsfdataz[0];
                        ...
                        if (tsfdata instanceof Number) {
                            ...
                            for (Map.Entry<String, String> entry : machine.components()
                                .entrySet()) {
                                if (tsfdata == null || tsfdata instanceof String && entry.getValue()
                                    .equals(tsfdata)
                                    || tsfdata instanceof UUID && entry.getKey()
                                        .equals(((UUID) tsfdata).toString())) {
                                    info++;
    ...
    public void load(NBTTagCompound nbt) {
        if (nbt.hasKey("compsel")) {
            ...
            for (int i = 0; i < components.length; i++) {
                if (compTag.hasKey("comp" + i)) delayConnectNBT[i] = compTag.getCompoundTag("comp" + i);
            }
        }
    }
    ...
    public void save(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < components.length; i++) {
            ...
            if (component != null) {
                ...
                if (component instanceof ThistleWrapper) ((ThistleWrapper) component).save(nbt);
                deviceTag.setString(
                    "_address",
                    getEnvironment(i).node()
                        .address());
    ...
    public void setBus(Bus bus) {
        super.setBus(bus);
        this.machine = (Machine) getBus().getMachine()
            .getContext();
    }
