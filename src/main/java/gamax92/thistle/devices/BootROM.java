...
public class BootROM extends Device {
    ...
    private boolean checkEEPROM() {
        if (this.eepromAddress != null && this.machine.components()
            .containsKey(this.eepromAddress)) return true;
    ...
    public int read(int address) {
        if (!checkEEPROM()) return 0;
        Node node = this.machine.node()
            .network()
            .node(this.eepromAddress);
        if (node == null) return 0;
        EEPROM eeprom = (EEPROM) node.host();
        if (address < 256) {
            byte[] volatileData = eeprom.volatileData();
            if (address < volatileData.length) return volatileData[address];
            else return 0;
        } else {
            address -= 256;
            byte[] codeData = eeprom.codeData();
            if (address < codeData.length) return codeData[address];
            else return 0;
        }
    }
    ...
    public void write(int address, int data) {
        ...
        if (address < 256) {
            byte[] volatileData = eeprom.volatileData();
            if (address < volatileData.length) volatileData[address] = (byte) data;
            else {
                byte[] extendedData = new byte[Settings.get()
                    .eepromDataSize()];
            ...
            if (eeprom.readonly()) return;
            address -= 256;
            byte[] codeData = eeprom.codeData();
            if (address < codeData.length) codeData[address] = (byte) data;
            else {
                byte[] extendedData = new byte[Settings.get()
                    .eepromSize()];
    ...
    public void load(NBTTagCompound nbt) {
        if (nbt.hasKey("rom")) {
            ...
            if (romTag.hasKey("eeprom")) this.eepromAddress = romTag.getString("eeprom");
        }
    }
    ...
    public void save(NBTTagCompound nbt) {
        ...
        if (this.eepromAddress != null) romTag.setString("eeprom", this.eepromAddress);
