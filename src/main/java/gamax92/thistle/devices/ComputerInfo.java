...
public class ComputerInfo extends Device {
    ...
    public void load(NBTTagCompound nbt) {
        if (nbt.hasKey("compinfo")) {
            ...
            if (computerUUID.length == 16) this.computerUUID = computerUUID;
            byte[] tmpfsUUID = infoTag.getByteArray("tmpfs");
            if (tmpfsUUID.length == 16) this.tmpfsUUID = computerUUID;
        }
    }
    ...
    public void setBus(Bus bus) {
        super.setBus(bus);
        Machine machine = (Machine) bus.getMachine()
            .getContext();

        computerUUID = UUIDHelper.encodeUUID(
            machine.node()
                .address());
