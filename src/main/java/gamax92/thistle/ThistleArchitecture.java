...
public class ThistleArchitecture implements Architecture {
    ...
    public boolean initialize() {
        ...
        int memory = calculateMemory(
            this.machine.host()
                .internalComponents());
        vm.machine.resize(memory);
        vm.machine.reset();
        for (ItemStack component : machine.host()
            .internalComponents()) {
            if (Driver.driverFor(component) instanceof Processor) {
                vm.cyclesPerTick = ThistleConfig.debugCpuSlowDown ? 10
                    : (Driver.driverFor(component)
                        .tier(component) + 1) * ThistleConfig.clocksPerTick;
                break;
            }
        }
        try {
    ...
    public Object[] invoke(String address, String method, Object[] args) throws Exception {
        if (!inSynchronizedCall) {
            ...
            if (node instanceof Component) {
                ...
                if (callback != null && !callback.direct()) throw new CallSynchronizedException(address, method, args);
            }
        }
        try {
    ...
    public Object[] invoke(Value value, String method, Object[] args) throws Exception {
        if (!inSynchronizedCall) {
            Option<Callbacks.Callback> option = Callbacks.apply(value)
                .get(method);
            if (option != null) {
                Callbacks.Callback callback = option.get();
                if (callback != null && !callback.annotation()
                    .direct()) throw new CallSynchronizedException(value, method, args);
            }
        }
        try {
    ...
    public void load(NBTTagCompound nbt) {
        ...
        if (gzmem.length > 0) {
            try {
                ...
                for (int i = 0; i < mem.length; i++) vm.machine.writeMem(i, mem[i]);
            } catch (IOException e) {
        ...
        // Restore Values
        if (nbt.hasKey("values")) ValueManager.load(nbt.getCompoundTag("values"), this.machine);
    ...
    public void save(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < mem.length; i++) mem[i] = vm.machine.readMem(i);
        try {
