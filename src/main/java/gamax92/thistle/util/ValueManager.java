...
public class ValueManager {
    ...
    private static void addValue(BiMap<Integer, Value> valuemap, Value value) {
        ...
        while (valuemap.containsKey(id)) id = (int) ((Math.random() * 2 - 1) * -((double) Integer.MIN_VALUE));
    ...
    public static int getID(Value value, Context context) {
        ...
        if (!valuemap.containsValue(value)) addValue(valuemap, value);
        return valuemap.inverse()
            .get(value);
    }

    public static void removeValue(Value value, Context context) {
        value.dispose(context);
        getValueMap(context).inverse()
            .remove(value);
    }
