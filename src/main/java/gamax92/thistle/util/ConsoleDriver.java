...
public class ConsoleDriver {
    ...
    private void updateComponents() {
        if (gpuADDR != null && this.machine.node()
            .network()
            .node(gpuADDR) == null) gpuADDR = null;
        if (screenADDR != null && this.machine.node()
            .network()
            .node(screenADDR) == null) screenADDR = null;
        if (gpuADDR == null || screenADDR == null) {
            canWrite = false;
            cursor = false;
            for (Map.Entry<String, String> entry : this.machine.components()
                .entrySet()) {
                String address = entry.getKey();
                String type = entry.getValue();
                if (type.equals("gpu") && gpuADDR == null) gpuADDR = address;
                if (type.equals("screen") && screenADDR == null) screenADDR = address;
            }
            if (gpuADDR != null && screenADDR != null) {
                canWrite = true;
                // Attempt to setup the GPU
                for (int i = -6; i <= -1; i++) databuf.addFirst(i);
                setup = true;
            }
        }
    }
    ...
    private void setString(NBTTagCompound nbt, String key, String value) {
        if (value != null) nbt.setString(key, value);
    }
    ...
    public void load(NBTTagCompound nbt) {
        ...
        if (nbt.hasKey("console")) {
            ...
            this.databuf.clear();
            for (int v : databuf) this.databuf.add(v);
        }
    }
    ...
    public void save(NBTTagCompound nbt) {
        ...
        int i = 0;
        for (int v : this.databuf) databuf[i++] = v;
            ...
            if (!parseANSI) {
                ...
                if (ansiDetect) {
                    ...
                    if (character == 91) {
                        ...
                        // It wasn't an ANSI escape at all, forward the inhibited 27 character and process this
                        // character normally.
                        databuf.add(27);
                    }
                }
                ...
                String[] ansiParts = this.ansiCode.toString()
                    .split(";", -1);
                try {
                    switch (character) {
                        case 'A':
                        ...
                        case 'H':
                            ...
                            int targetX = 1;
                            int targetY = 1;
                            if (ansiParts.length >= 1) targetY = clampH(Integer.parseInt(ansiParts[0]));
                            if (ansiParts.length == 2) targetX = clampW(Integer.parseInt(ansiParts[1]));
                        ...
                        case 'J':
                            if (ansiCode.length() == 0 || ansiCode.equals("0")) {
                                ...
                                databuf.add(-501);
                            } else if (ansiCode.equals("2")) databuf.add(-6);
                            break;
                        case 'K':
                            if (ansiCode.length() == 0 || ansiCode.equals("0")) databuf.add(-502);
                            else if (ansiCode.equals("1")) databuf.add(-503);
                            else if (ansiCode.equals("2")) databuf.add(-504);
                            break;
                        case 'L':
                            // TODO: Insert n lines
                            break;
                        ...
                        case 'X':
                            ...
                            break;
                        case 'h':
                            if (ansiCode.equals("?25")) databuf.add(-505);
                            break;
                        case 'l':
                            if (ansiCode.equals("?25")) databuf.add(-506);
                            break;
                        case 'm':
                            ...
                            for (String part : ansiParts) {
                                ...
                                if (ipart == 0) {
                                    ...
                                    newTextFG = 7;
                                    newTextBG = 0;
                                } else if (ipart == 1) newBrightFG = true;
                                else if (ipart == 5) newBrightBG = true;
                                else if (ipart >= 30 && ipart <= 37) newTextFG = ipart - 30;
                                else if (ipart >= 40 && ipart <= 47) newTextBG = ipart - 40;
                            }
                        ...
                        default:
                            databuf.add(27);
                            databuf.add(91);
                            for (int i = 0; i < ansiCode.length(); i++) databuf.add((int) ansiCode.charAt(i));
                    }
                ...
                // or an accident
                if (ansiCode.length() < 256) ansiCode.append((char) character);
            }
        }
    }
    ...
    public void flush() throws Exception {
        ...
        if (canWrite) {
            try {
                if (showCursor && ((System.currentTimeMillis() - this.lastTime) >= 500)
                    && !setup
                    && (X >= 1 && X <= W && Y >= 1 && Y <= H)) {
                ...
                while (!databuf.isEmpty()) {
                    ...
                    if ((character < -1004 || character > -1000) && cursor) {
                        // When performing any operation that isn't one of the 4 cursor toggle operations, temporarily
                        // hide the cursor.
                        // This implies that cursor control operations start with the cursor already hidden.
                        // This is written in such a way as to cause -1001 to be executed, followed by the rest of that
                        // command, before the original command.
                        databuf.addFirst(-1001);
                        character = -1001;
                    }
                    switch (character) {
                        ...
                        case -4:
                            machine.invoke(
                                gpuADDR,
                                "setBackground",
                                new Object[] { colors[(this.brightBG ? 8 : 0) + this.textBG] });
                            break;
                        case -5:
                            machine.invoke(
                                gpuADDR,
                        ...
                        case -1004: // Cursor 4. Set Character
                            ...
                            if (!this.cursor) {
                                ...
                                if (this.cursorFG != colors[(this.brightFG ? 8 : 0) + this.textFG]) chained.add(-5);
                            }
                            break;
                        ...
                        default:
                            machine.invoke(
                                gpuADDR,
                                "set",
                                new Object[] { this.X, this.Y, Character.toString((char) character) });
                            this.X = this.X + 1;
                            if (this.X > this.W) {
                                this.Y = this.Y + 1;
                                this.X = 1;
                    ...
                    for (int i = 0; i < cmdLen; i++) databuf.removeFirst();
