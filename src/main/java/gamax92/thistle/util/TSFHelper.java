...
public class TSFHelper {
    ...
    private static long readLong(Queue<Byte> buffer) {
        long value = b(buffer) | (b(buffer) << 8)
            | (b(buffer) << 16)
            | (b(buffer) << 24)
            | (b(buffer) << 32)
            | (b(buffer) << 40)
    ...
    private static byte[] readByteArray(Queue<Byte> buffer) {
        ...
        for (int i = 0; i < length; i++) data[i] = buffer.remove();
        return data;
    }

    private static UUID readUUID(Queue<Byte> buffer) {
        byte[] uuid = new byte[16];
        for (int i = 0; i < uuid.length; i++) uuid[i] = buffer.remove();
        char[] hex = Hex.encodeHex(uuid, true);
        StringBuilder address = new StringBuilder();
        for (int i = 0; i < hex.length; i++) {
            address.append(hex[i]);
            if (i == 7 || i == 11 || i == 15 || i == 19) address.append('-');
        }
    ...
    private static Map readMap(Queue<Byte> buffer, Context context, int conversion) {
        Map map = new HashMap();
        Object[] pairs = readArray(buffer, context, conversion);
        if ((pairs.length % 2) == 1) throw new IllegalArgumentException("key with no value");
    ...
    private static Object readValue(Queue<Byte> buffer, Context context) {
        ...
        if (value == null) throw new IllegalArgumentException("no Value available with id: " + id);
        return value;
    }
        ...
        try {
            while (true) {
                ...
                switch (tag) {
                    ...
                    case 12:
                        output.add(
                            ((conversion & 0x10) != 0) ? readArrayMap(buffer, context, conversion)
                                : readArray(buffer, context, conversion));
                        break;
                    case 13:
    ...
    private static void writeThing(Queue<Byte> buffer, Object thing, Context context, int conversion) {
        if (((conversion & 8) != 0) && thing instanceof String
            && uuidtest.matcher((String) thing)
                .matches())
        ...
        if (thing == null) writeNull(buffer);
        else if (thing instanceof Boolean) writeBoolean(buffer, (Boolean) thing);
        else if (thing instanceof Byte) writeByte(buffer, (Byte) thing);
        else if (thing instanceof Short) writeShort(buffer, (Short) thing);
        else if (thing instanceof Integer) writeInt(buffer, (Integer) thing);
        else if (thing instanceof Long) writeLong(buffer, (Long) thing);
        else if (thing instanceof Double) {
            double value = (Double) thing;
            if ((conversion & 2) != 0) writeFixed(buffer, value);
            else if ((conversion & 1) != 0) writeInt(buffer, (int) Math.floor(value));
            else writeDouble(buffer, value);
        } else if (thing instanceof byte[]) writeByteArray(buffer, (byte[]) thing);
        else if (thing instanceof Byte[]) writeByteArray(buffer, ArrayUtils.toPrimitive((Byte[]) thing));
        else if (thing instanceof String) writeString(buffer, (String) thing);
        else if (thing instanceof UUID) writeUUID(buffer, (UUID) thing);
        else if (thing instanceof Object[]) {
            buffer.add((byte) 12);
            writeArray(buffer, (Object[]) thing, context, conversion);
            writeEnd(buffer);
        } else if (thing instanceof Map) writeMap(buffer, (Map) thing, context, conversion);
        else if (thing instanceof Value) writeValue(buffer, (Value) thing, context);
        else Thistle.log.warn(
            "Don't know how to TSF encode a " + thing.toString()
                + " ("
                + thing.getClass()
                    .getName()
                + "), please report this to Thistle's author.");
    }
    ...
    public static void writeUUID(Queue<Byte> buffer, UUID uuid) {
        ...
        for (int i = 0; i < data.length; i++) buffer.add(data[i]);
    }
