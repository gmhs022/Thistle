...
public class GenericDevice extends ThistleWrapper {
    ...
    private Cleanup cleanup = new Cleanup() {
        ...
        public void run(Object[] results, Context context) {
            status = 0;
            if (ThistleConfig.debugComponentCalls) Thistle.log.info(
                "[Generic] (" + host().node()
                    .address() + ") Results: " + Arrays.deepToString(results));
            if (results != null) {
    ...
    public int readThistle(Context context, int address) {
        if (address == GENERIC_STATUS_REG) {
            ...
            if (namepos < name.length()) return name.charAt(namepos);
            else return 0;
        } else if (address == GENERIC_SLOT_REG) {
            return ((Machine) context).host()
                .componentSlot(
                    host().node()
                        .address());
        } else {
            ...
            case GENERIC_STATUS_REG:
                ...
                switch (data) {
                    case 0: // invoke
                        ...
                        if (ThistleConfig.debugComponentCalls) Thistle.log.info(
                            "[Generic] (" + host().node()
                                .address() + ") Invoke: " + Arrays.deepToString(tsfdata));
                        ...
                        try {
                            ...
                            if (tsfdata.length >= 1 && tsfdata[0] instanceof String) {
                                ...
                                results = thistle.invoke(
                                    host().node()
                                        .address(),
                                    (String) tsfdata[0],
                                    args);
                            } else if (tsfdata.length >= 2 && tsfdata[0] instanceof Value
                                && tsfdata[1] instanceof String) {
                    ...
                    case 3: // documentation
                        ...
                        Callback callback = ((Machine) context).methods(host())
                            .get(tsfdata[0]);
                        if (callback != null) {
    ...
    public void load(NBTTagCompound nbt) {
        ...
        if (nbt.hasKey("name")) this.name = nbt.getString("name");
        inputbuf.clear();
    ...
    public void save(NBTTagCompound nbt) {
        ...
        if (this.name != null) nbt.setString("name", this.name);
