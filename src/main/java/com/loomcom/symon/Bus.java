...
public class Bus {
    ...
    public int read(int address) {
        ...
        if (address < 0xE000 || address >= EEPROM_CODE_BASE) {
            ...
            int bankMask = machine.getComponentSelector()
                .getMask();
            if (select == 15 && (bankMask & (1 << 4)) == 0) {
                if (ThistleConfig.debugEEPROMReads) Thistle.log.info(String.format("[Bus] EEPROM Read $%04X", address));
                return machine.getEEPROM()
                    .read(address - EEPROM_DATA_BASE) & 0xFF; // This reads EEPROM code but offset must be data
                ...
                IThistleDevice device = machine.getComponentSelector()
                    .getComponent(index);
                if (device != null) return device.readThistle(machine.getContext(), address & 0xFF) & 0xFF;
            } else {
                if (ThistleConfig.debugMemoryReads) Thistle.log.info(String.format("[Bus] Memory Read $%04X", address));
                BankSelector banksel = machine.getBankSelector();
                int memaddr = (banksel.bankSelect[select] << 12) | (address & 0xFFF);
                if (memaddr < machine.getMemsize()) return machine.readMem(memaddr) & 0xFF;
            }
            return 0;
        }
        if (ThistleConfig.debugDeviceReads) Thistle.log.info(String.format("[Bus] Device Read $%04X", address));
    ...
    public void write(int address, int data) {
        ...
        if (address < 0xE000 || address >= EEPROM_CODE_BASE) {
            ...
            if (select == 15 && (bankMask & (1 << 4)) == 0) {
                ...
                machine.getEEPROM()
                    .write(address - EEPROM_DATA_BASE, data); // This reads eeprom code but offset must be data
            } else if (select >= 10 && select <= 13 && (bankMask & (1 << (select - 10))) == 0) {
                ...
                IThistleDevice device = machine.getComponentSelector()
                    .getComponent(index);
                if (device != null) device.writeThistle(machine.getContext(), address & 0xFF, data);
            } else {
                if (ThistleConfig.debugMemoryWrites)
                    Thistle.log.info(String.format("[Bus] Memory Write $%04X = 0x%02X", address, data));
                BankSelector banksel = machine.getBankSelector();
                int memaddr = (banksel.bankSelect[select] << 12) | (address & 0xFFF);
                if (memaddr < machine.getMemsize()) machine.writeMem(memaddr, (byte) data);
            }
            return;
        }
    ...
    public void onSignal(Signal signal) {
        for (Device device : deviceList) device.onSignal(signal);
    }

    public void load(NBTTagCompound nbt) {
        for (Device device : deviceList) device.load(nbt);
    }

    public void save(NBTTagCompound nbt) {
        for (Device device : deviceList) device.save(nbt);
    }
